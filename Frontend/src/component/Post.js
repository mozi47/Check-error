...
import {singlePost, deletePostAction, likePostAction} from "../actions/PostAction"
...
const Post = ({post}) => {
    ...
    return (
      ...
      {(user?.result?.googleId === post.user || user?.result?._id === post.user ) && (
        <>
        <Button style={{ color: 'white' }} size="small" onClick={()=>{dispatch(singlePost(post._id))}}><MoreHorizIcon fontSize="default" /></Button>
        </>
      )}
      </div>
      ...
      <CardActions className={classes.cardActions}>
          <Button size="small" color="primary" disabled={!user?.result} onClick={()=> dispatch(likePostAction(post._id)) }> <Likes /> </Button>
        {(user?.result?.googleId === post.user || user?.result?._id === post.user ) && (
          <>
          <Button size="small" color="primary" onClick={()=> dispatch(deletePostAction(post._id)) }><DeleteIcon fontSize="small" /> Delete</Button>
          </>
        )}
      </CardActions>
    </Card>
